...
function itemSummary(items?: Array<{ title?: string; label?: string }>) {
  if (!items?.length) return "Not added yet";
  const first = items[0];
  return first.title || first.label || `${items.length} item${items.length === 1 ? "" : "s"}`;
}
...
function Area({ label, value, setValue, placeholder, rows = 3 }: { label: string; value: string; setValue: (value: string) => void; placeholder?: string; rows?: number }) {
    ...
    <div className="flex items-center justify-between gap-4 border-b py-5 last:border-b-0">
      <div className="flex min-w-0 gap-3">
        ...
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2"><h3 className="font-semibold leading-tight">{title}</h3>{!!count && <Badge variant="outline">{count}</Badge>}</div>
