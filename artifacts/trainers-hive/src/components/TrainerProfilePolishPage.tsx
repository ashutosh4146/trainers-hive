...
const LANGUAGE_OPTIONS = ["English", "Hindi", "Marathi", "Gujarati", "Punjabi", "Bengali", "Tamil", "Telugu", "Kannada", "Malayalam", "Urdu", "Odia", "Assamese", "Spanish", "French", "German", "Arabic", "Japanese", "Mandarin"];
...
function CompletionTile({ label, done, helper }: { label: string; done: boolean; helper: string }) {
  return <div className="flex items-start justify-between gap-3 rounded-xl border bg-background p-3 text-sm"><div><p className="font-medium">{label}</p><p className="mt-1 text-xs text-muted-foreground">{helper}</p></div>{done ? <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-emerald-600" /> : <span className="mt-0.5 shrink-0 rounded-full border px-2 py-0.5 text-[11px] text-muted-foreground">Pending</span>}</div>;
}

function MetricCard({ label, value, helper, icon }: { label: string; value: React.ReactNode; helper: string; icon: React.ReactNode }) {
  return <Card className="border-primary/10"><CardContent className="p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-medium text-muted-foreground">{label}</p><p className="mt-2 text-2xl font-bold leading-none">{value}</p><p className="mt-2 text-xs leading-relaxed text-muted-foreground">{helper}</p></div><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-primary/10 text-primary">{icon}</span></div></CardContent></Card>;
}
...
function LanguagePicker({ value, onChange }: { value: string[]; onChange: (next: string[]) => void }) {
  ...
  const addLanguage = (language: string) => {
    ...
    if (value.some((item) => item.toLowerCase() === clean.toLowerCase())) { setQuery(""); return; }
  ...
  return <div className="space-y-3"><div className="flex gap-2"><Input value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); addLanguage(suggestions[0] || query); } }} placeholder="Type 2 letters, e.g. en, hi, ma" /><Button type="button" variant="outline" onClick={() => addLanguage(suggestions[0] || query)} disabled={cleanQuery.length < 2}>Add</Button></div>{suggestions.length > 0 && <div className="flex flex-wrap gap-2">{suggestions.map((language) => <button key={language} type="button" onClick={() => addLanguage(language)} className="rounded-full border bg-background px-3 py-1 text-sm text-muted-foreground transition-colors hover:border-primary hover:text-primary">{language}</button>)}</div>}<div className="flex min-h-10 flex-wrap gap-2 rounded-xl border bg-background p-2">{value.length === 0 ? <span className="px-2 py-1 text-sm text-muted-foreground">No languages added yet</span> : value.map((language) => <Badge key={language} variant="secondary" className="gap-1 rounded-full px-3 py-1">{language}<button type="button" onClick={() => onChange(value.filter((item) => item !== language))} aria-label={`Remove ${language}`}><X className="h-3 w-3" /></button></Badge>)}</div><p className="text-xs text-muted-foreground">Start typing at least 2 letters to see full language suggestions, then click Add.</p></div>;
}
...
function CertEditor({ value, onChange }: { value: Cert[]; onChange: (next: Cert[]) => void }) {
  ...
  const add = () => {
    ...
    if (!cleanName) { toast({ title: "Certification name required", variant: "destructive" }); return; }
    if (cleanUrl) { try { new URL(cleanUrl); } catch { toast({ title: "Invalid URL", description: "Use a valid certificate link or leave it blank.", variant: "destructive" }); return; } }
    onChange([...value, { name: cleanName, ...(cleanUrl ? { url: cleanUrl } : {}) }]);
    setName(""); setUrl("");
  };
  return <div className="space-y-3"><div className="grid gap-3 md:grid-cols-[1fr_1fr_auto]"><Input value={name} onChange={(event) => setName(event.target.value)} placeholder="Certification name" /><Input value={url} onChange={(event) => setUrl(event.target.value)} placeholder="Verification link optional" /><Button type="button" variant="outline" onClick={add}>Add</Button></div>{value.length > 0 && <div className="space-y-2">{value.map((cert, index) => <div key={`${cert.name}-${index}`} className="flex items-center justify-between gap-3 rounded-xl border p-3 text-sm"><div className="min-w-0"><p className="truncate font-medium">{cert.name}</p>{cert.url && <a href={cert.url} target="_blank" rel="noopener noreferrer" className="mt-1 inline-flex items-center gap-1 text-xs text-primary hover:underline"><ExternalLink className="h-3 w-3" /> View proof</a>}</div><Button type="button" variant="ghost" size="icon" onClick={() => onChange(value.filter((_, itemIndex) => itemIndex !== index))} aria-label="Remove certification"><X className="h-4 w-4" /></Button></div>)}</div>}</div>;
}
...
export function TrainerProfilePolishPage({ trainerId, registeredEmail }: { trainerId: string; registeredEmail: string }) {
  ...
  const form = useForm<TrainerFormValues>({ resolver: zodResolver(trainerSchema), defaultValues: { name: "", headline: "", mainSkill: "", experienceYears: 0, developmentExperienceYears: 0, location: "", bio: "", trainerType: undefined as unknown as "trainer" | "developer" | "both", resumeUrl: "" } });
  ...
  React.useEffect(() => {
    if (!trainerId) return;
    customFetch<ProfileExtrasResponse>(`/api/trainers/${trainerId}/profile-extras`).then((data) => { const extras = data.profileExtras ?? {}; setProfileExtras(extras); setMobileNumber(extras.mobileNumber ?? ""); setAvatarUrl(data.avatarUrl ?? ""); }).catch(() => {});
  }, [trainerId]);

  const watched = form.watch();
  const subSkills = splitTags(subSkillsText);
  const skillSuggestions = React.useMemo(() => { const items: string[] = []; for (const group of skillsData ?? []) for (const skill of group.skills) items.push(skill); return items.slice(0, 300); }, [skillsData]);
  ...
  const onSubmit = (data: TrainerFormValues) => {
    if (languages.length === 0) { toast({ title: "Language required", description: "Add at least one language you train in.", variant: "destructive" }); return; }
    updateTrainer.mutate({ id: trainerId, data: { ...data, headline: (data.headline ?? "").trim(), resumeUrl: (data.resumeUrl ?? "").trim(), subSkills, languages, certifications } }, {
      onSuccess: async () => { const nextExtras = { ...profileExtras, mobileNumber: mobileNumber.trim() }; setProfileExtras(nextExtras); try { await customFetch(`/api/trainers/${trainerId}/profile-extras`, { method: "PATCH", body: JSON.stringify({ avatarUrl, profileExtras: nextExtras }) }); } catch { toast({ title: "Profile saved, but mobile number was not updated", variant: "destructive" }); } toast({ title: "Profile updated", description: "Your trainer profile has been saved." }); queryClient.invalidateQueries({ queryKey: getGetTrainerQueryKey(trainerId) }); queryClient.invalidateQueries({ queryKey: getGetCurrentUserQueryKey() }); },
      onError: (err) => { const anyErr = err as { response?: { data?: { error?: string } } }; toast({ title: "Could not save profile", description: anyErr?.response?.data?.error ?? "Please check your inputs and try again.", variant: "destructive" }); },
    });
  };
