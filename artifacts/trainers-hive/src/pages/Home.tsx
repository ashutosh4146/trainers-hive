...
import { ArrowRight, BookOpen, Briefcase, Building, CalendarDays, Clock, ClipboardList, FileSignature, MapPin, MessageSquare, Sparkles, PlusCircle } from "lucide-react";
...
function isDeadlineSoon(value: unknown) {
  if (!value || hasDeadlinePassed(value)) return false;
  const deadline = new Date(String(value));
  const today = new Date(); today.setHours(0, 0, 0, 0);
...
export default function Home() {
  ...
  const postRequirementHref = auth?.role === "vendor" ? "/requirements/new" : "/login?redirect=/requirements/new";
  ...
  return (
    <div className="w-full flex flex-col">
      <section className="relative w-full overflow-hidden border-b border-primary/20 bg-slate-900 py-20 text-slate-50 md:py-32 lg:py-40">
        ...
        <div className="container relative z-10 mx-auto px-4 md:px-6"><div className="max-w-3xl"><Badge variant="outline" className="mb-6 border-primary/50 bg-primary/10 px-3 py-1 text-primary-foreground backdrop-blur-sm dark:text-white">The B2B Training Marketplace</Badge><h1 className="mb-6 text-4xl font-extrabold leading-tight tracking-tight md:text-6xl lg:text-7xl">Source top-tier <span className="text-primary">expert trainers</span> for your institution.</h1><p className="mb-10 max-w-2xl text-lg leading-relaxed text-slate-300 md:text-xl">Trainers Hive is the verified infrastructure layer connecting corporate L&amp;D leaders, universities, and specialized training professionals for high-stakes engagements.</p><div className="flex flex-wrap gap-4"><Link href="/requirements"><Button size="lg" className="h-12 px-8 text-base shadow-lg shadow-primary/20"><BookOpen className="mr-2 h-5 w-5" /> Browse Requirements</Button></Link><Link href={postRequirementHref}><Button size="lg" variant="outline" className="h-12 border-slate-700 bg-slate-800/50 px-8 text-base text-white backdrop-blur-sm hover:bg-slate-800 hover:text-white"><PlusCircle className="mr-2 h-5 w-5" /> Post Requirement</Button></Link>{isAdmin && <Link href="/trainers"><Button size="lg" variant="outline" className="h-12 border-slate-700 bg-slate-800/50 px-8 text-base text-white backdrop-blur-sm hover:bg-slate-800 hover:text-white">View Trainer Directory</Button></Link>}</div></div></div>
      </section>
