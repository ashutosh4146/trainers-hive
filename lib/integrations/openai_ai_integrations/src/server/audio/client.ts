...
export async function voiceChat(
  ...
  const response = await openai.chat.completions.create({
    ...
    messages: [{ role: "user", content: [{ type: "input_audio", input_audio: { data: audioBuffer.toString("base64"), format: inputFormat } }] }],
  });
  ...
  const transcript = (audio?.transcript as string | undefined) ?? (msg?.content as string | undefined) ?? "";
...
export async function voiceChatStream(
  ...
  return (async function* () {
    for await (const chunk of stream) {
      ...
      if (!delta) continue;
      const deltaAudio = delta.audio as Record<string, unknown> | undefined;
      if (deltaAudio?.transcript) yield { type: "transcript", data: deltaAudio.transcript as string };
      if (deltaAudio?.data) yield { type: "audio", data: deltaAudio.data as string };
    }
  })();
}
...
export async function textToSpeech(
  ...
  const msg = response.choices[0]?.message as unknown as Record<string, unknown>;
  const audioData = (msg?.audio as Record<string, unknown> | undefined)?.data ?? "";
...
export async function textToSpeechStream(
  ...
  return (async function* () {
    for await (const chunk of stream) {
      const delta = chunk.choices?.[0]?.delta as unknown as Record<string, unknown> | undefined;
      const deltaAudio = delta?.audio as Record<string, unknown> | undefined;
      if (deltaAudio?.data) yield deltaAudio.data as string;
    }
  })();
}
